...
class Drama {
	...
	void getInfomation () {
		...
		for (int i = 0; i < actors.length; i++) {
			...
			/* 삼항연산자도 사용가능
			* i < actors.length -1 / true - actors[i] + ", " / false - actors[i]
			* System.out.print(i < actors.length -1 ? actors[i] + ", " : actors[i]);
			*/
			/* 변수에 담아서 사용
...
class TriangleMath {
	...
	static double getHypotenuse (int bottom, int height) {
		...
		// if (!(bottom > 0 && height > 0)) return 0; - and 사용 가능 
		
		double result = Math.pow(bottom, 2) + Math.pow(height, 2); // 제곱
		return Math.sqrt(result); // 제곱근	
	}
	
	static double getCircumference (int height, int bottom, int hypotenuse) {
		if (hypotenuse <= 0 || height <= 0 || bottom <= 0) return 0;
...
//		new GalaxyPhone(); // -> 인스턴스 생성하는 구문
		...
		GalaxyPhone.maker = "SAMSUNG"; // static
...
//		인스턴스 : new Drama()
		...
		System.out.println("빗변 : " + bit);
		
		// System.err.println(TriangleMath.PI);
		// TriangleMath.PI 단일책임원칙에 위배 PI는 TriangleMath와 관련이 없음
		// TriangleMath.PI = 0.0; - Error 변경 불가
