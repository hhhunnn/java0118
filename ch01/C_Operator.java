...
public class C_Operator {
	...
	public static void main(String[] args) {
		...
		// 결과부터 계산
		number *= 7 - 5; // 26
		// number *= (7 - 5); - 먼저 계산할 구간 괄호 사용
		System.out.println(number);
		
		// int number1;
		// number1 += 10; - 초기화가 되어있지 않아서 오류
		...
		// || : OR 연산 - 좌항과 우항 중 하나라도 true이면 true, 모두 false면 false
		booleanResult = true || true;
		System.out.println(booleanResult);
		booleanResult = a < b || c > d;
		System.out.println(booleanResult);
		...
		// a: 15, b: 7
		// &&의 앞의 결과가 false일 경우, && 뒤의 식을 계산하지 않는다는 증명(false)
		// && 뒤의 결과를 계산하면 b의 값은 8이 되어야 함
		booleanResult = (a < b) && (a > ++b);
		System.out.println(booleanResult);
		System.out.println(b); // 7 출력
		
		// a: 15, b: 7
		// ||의 앞의 결과가 true일 경우, || 뒤의 식을 계산하지 않는다는 증명(true)
		// || 뒤의 결과를 계산하면 b의 값은 8이 되어야 함
		booleanResult = (a > b) || (a > ++b);
		System.out.println(booleanResult);
		System.out.println(b); // 7 출력
