...
public class B_DataType {
	...
	public static void main(String[] args) {
		...
		// short : 2byte(16bit)의 크기를 가진는 정수형 데이터 타입
		// -32768 ~ 32767
		short short1 = 32767;
		// short short2 = 32768;
		...
		number = 256; //128 데이터 손실
		byte byteNumber = (byte) number;
		System.out.println(byteNumber); // -128 출력됨 : 데이터 손실
		
		short shortNumber = 97;
		char character = 97;
		// shortNumber = character; - 오류
		
		int intNumber = 100;
		float floatNumber = 100;
		// intNumber = floatNumber; - 오류
		...
		int[] numbers;
		numbers = new int[5];
		System.out.println(numbers); // 메모리 주소가 출력됨
		...
		numbers[4] = -99;
		// numbers : {99, 0, 0, 0, -99}
		// numbers2 : {99, 0, 0, 0, 0}X  {99, 0, 0, 0, -99}o
		System.out.println(numbers2[4]); // -99
		// 주소를 참조하기 때문에 numbers가 4인덱스가 -99로 재할당 되면 numbers2도 재할당 됨
		...
		System.out.println(number2); // 이후에 number1이 재할당되도 number2가 재할당 되지않음
		...
		System.out.println(statement.indexOf("하마")); // 존재하지 않으면 음수
		System.out.println(statement.lastIndexOf("그림"));
		System.out.println(statement.lastIndexOf("하마")); // 존재하지 않으면 음수
		...
		// - 참조형 데이터 타입에서만 null 사용 가능
		String string2 = null;
		System.out.println(string2);
		// System.out.println(string2.substring(48)); - 오류 NullPointerException
